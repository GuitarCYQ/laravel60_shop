...
class MerchantAddressModel extends Model
{
    //表名
    ...
    public function getByCondition( $condition = array(),  $type = '*',  $groupBy = '',  $orderBy = array(),  $page = 0,  $pageSize = 20)
    {
        ...
        if (isset($condition['ma_id']) && !empty($condition['ma_id']))
        {
            if (is_array($condition['ma_id'])) {
                $query->whereIn('ma_id', $condition['ma_id']);
            } else {
                $query->where('ma_id', $condition['ma_id']);
            }
        }
        if (isset($condition['merchant_id']) && !empty($condition['merchant_id'])) $query->where('merchant_id', $condition['merchant_id']);
        if (isset($condition['name']) && !empty($condition['name'])) $query->where('name', 'like' ,'%'.$condition['name'].'%');
